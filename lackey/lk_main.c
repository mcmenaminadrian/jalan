...
/* Lackeml thread callbacks */
static void lackeyml_start_client_code_callback(ThreadId tid, ULong blocks_done)
{
   VG_(printf)("<thread tid=\"%08lx\">\n", tid);
}

static void lackeyml_stop_client_code_callback(ThreadId tid, ULong blocks_done)
{
   VG_(printf)("</thread>\n");
}
...
static Bool lk_process_cmd_line_option(const HChar* arg)
{
   ...
   if (clo_trace_mem) {
	...
	VG_(printf)("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
        ...
        VG_(printf)(" \"http://cartesianproduct.wordpress.com\">\n");
	...
	VG_(printf)("<!ATTLIST thread address CDATA #REQUIRED>\n");
        ...
        VG_(printf)("]>\n");
        VG_(printf)("<lackeyml");
	VG_(printf)(" xmlns=\"http://cartesianproduct.wordpress.com\">\n");

	//set up callback
	VG_(track_start_client_code) ( & lackeyml_start_client_code_callback );
	VG_(track_stop_client_code) ( & lackeyml_stop_client_code_callback );
   }
   return True;
}
...
{
   VG_(printf)("<instruction address=\"%08lx\" size=\"%02lx\" />\n", addr, size);
}

static VG_REGPARM(2) void trace_load(Addr addr, SizeT size)
{
   VG_(printf)("<load address=\"%08lx\" size=\"%02lx\" />\n", addr, size);
}

static VG_REGPARM(2) void trace_store(Addr addr, SizeT size)
{
   VG_(printf)("<store address=\"%08lx\" size=\"%02lx\" />\n", addr, size);
}

static VG_REGPARM(2) void trace_modify(Addr addr, SizeT size)
{
   VG_(printf)("<modify address=\"%08lx\" size=\"%02lx\" />\n", addr, size);
}
...
static void lk_fini(Int exitcode)
{
   ...
   if (!clo_trace_mem && clo_detailed_counts) {
      VG_(umsg)("\n");
      VG_(umsg)("IR-level counts by type:\n");
      print_details();
   }

   if (!clo_trace_mem && clo_basic_counts) {
      VG_(umsg)("\n");
